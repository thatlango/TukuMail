...
pub async fn create_mailbox(
    ...
    let domain = address.split_once('@').unwrap().1;
    let exists: bool = sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM engine_v2_domains WHERE name=$1)")
        .bind(domain)
        .fetch_one(pool)
        .await?;
    if !exists {
...
pub async fn authenticate(pool: &PgPool, address: &str, password: &str) -> Result<bool> {
    ...
    let row = sqlx::query("SELECT password_hash,suspended FROM engine_v2_mailboxes WHERE address=$1")
        .bind(address)
        .fetch_optional(pool)
        .await?;
...
pub async fn send(
    ...
    for recipient in to.iter().chain(cc.iter()) {
        if let Some(local) = resolve_local(pool, recipient).await? {
            store_message(pool, &local, &sender, to, cc, subject, body).await?;
        } else {
    ...
    if !remote.is_empty() {
        sqlx::query(
            "INSERT INTO engine_v2_outgoing_queue(id,sender,recipients,cc,subject,body)
             VALUES ($1,$2,$3,$4,$5,$6)",
        )
        .bind(Uuid::new_v4())
        .bind(sender)
...
pub async fn accept_smtp_message(
    pool: &PgPool,
    envelope_from: &str,
    recipients: &[String],
    subject: &str,
    ...
    for recipient in recipients {
        if let Some(local) = resolve_local(pool, recipient).await? {
            store_message(pool, &local, &sender, recipients, &[], subject, body).await?;
            local_count += 1;
        } else {
            remote.push(normalise_address(recipient)?);
        }
...
async fn store_message(
    ...
    let used: i64 = sqlx::query_scalar(
        "SELECT COALESCE(SUM(size_bytes),0)::BIGINT FROM engine_v2_messages WHERE mailbox_address=$1",
    )
    .bind(mailbox)
    .fetch_one(pool)
    .await?;
    let size = (subject.len() + body.len() + sender.len() + 256) as i64;
    ...
    sqlx::query(
        "INSERT INTO engine_v2_messages
         (id,mailbox_address,sender,recipients,cc,subject,body,size_bytes)
...
pub async fn imap_messages(pool: &PgPool, address: &str) -> Result<Vec<StoredMessage>> {
    let address = normalise_address(address)?;
    let rows = sqlx::query(
        "SELECT id,uid,sender,recipients,cc,subject,body,received_at,seen
         FROM engine_v2_messages
         WHERE mailbox_address=$1 AND folder='INBOX'
         ORDER BY uid ASC",
    )
...
pub async fn queue_due(pool: &PgPool, limit: i64) -> Result<Vec<QueueItem>> {
    let rows = sqlx::query(
        "SELECT id,sender,recipients,cc,subject,body,attempts
    ...
    .fetch_all(pool)
    .await?;
    Ok(rows.into_iter().map(|r| QueueItem {
        id: r.get("id"),
        sender: r.get("sender"),
        recipients: r.get("recipients"),
        cc: r.get("cc"),
...
pub async fn queue_delivered(pool: &PgPool, id: Uuid) -> Result<()> {
    sqlx::query(
        "UPDATE engine_v2_outgoing_queue SET state='DELIVERED', delivered_at=now(), last_error=NULL WHERE id=$1",
    )
    .bind(id)
    .execute(pool)
    .await?;
...
pub async fn queue_failed(pool: &PgPool, item: &QueueItem, error: &str) -> Result<()> {
    ...
    let delay_minutes = 2_i64.saturating_pow(attempts.min(10) as u32).min(24 * 60);
...
pub fn normalise_address(raw: &str) -> Result<String> {
    let value = raw.trim().trim_matches('<').trim_matches('>').to_ascii_lowercase();
