...
async fn handle(stream: TcpStream, pool: PgPool) -> Result<()> {
    ...
    loop {
        ...
        if upper.starts_with("EHLO ") || upper.starts_with("HELO ") {
            ...
            if !state.helo {
                write
                    .write_all(b"503 5.5.1 Send HELO/EHLO first\r\n")
                    .await?;
                continue;
            }

            let raw = command["MAIL FROM:".len()..]
                .split_whitespace()
                .next()
                .unwrap_or("");
            ...
            match db::normalise_address(raw) {
                Ok(address) => {
                    if let Some(authenticated) = state.authenticated.as_ref()
                        && authenticated != &address
                    {
                        write
                            .write_all(b"553 5.7.1 Authenticated user cannot spoof MAIL FROM\r\n")
                            .await?;
                        continue;
                    }
            ...
            .await
            {
                ...
                Err(error) => {
                    ...
                    write
                        .write_all(b"451 4.3.0 Temporary processing failure\r\n")
                        .await?;
                }
            }
        } else if upper == "QUIT" {
